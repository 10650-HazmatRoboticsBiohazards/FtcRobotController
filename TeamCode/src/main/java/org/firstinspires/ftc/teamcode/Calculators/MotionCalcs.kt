        ...
        return object : MotionCalc {
            ...
            override fun CalcMotion(d: MoveData): Vector2D {
                if(!initialized){
                    ...
                    if(points.size > 1) {
                        segments.add(Straight(startPoint = d.wPos, endPoint = points[0].getSubtracted(d.wPos).getMultiplied(startTurnPercent).getAdded(d.wPos)))//can be complicated but for the sake of getting it working
                        segments.add(Curve(
                            startPoint = points[0].getSubtracted(d.wPos).getMultiplied(startTurnPercent).getAdded(d.wPos),
                            controlPoint = points[0],
                            endPoint = points[1].getSubtracted(points[0]).getMultiplied(1.0-startTurnPercent).getAdded(points[0])))
                    } else {
                    ...
                    for(i in 1..points.size-2){
                        segments.add(Straight(
                            startPoint = points[i].getSubtracted(points[i-1]).getMultiplied(1.0-startTurnPercent).getAdded(points[i-1]),
                            endPoint = points[i].getSubtracted(points[i-1]).getMultiplied(startTurnPercent).getAdded(points[i-1])))
                        segments.add(Curve(
                            startPoint = points[i].getSubtracted(points[i-1]).getMultiplied(startTurnPercent).getAdded(points[i-1]),
                            controlPoint = points[i],
                            endPoint = points[i+1].getSubtracted(points[i]).getMultiplied(1.0-startTurnPercent).getAdded(points[i])))
                    }

                    if(points.size > 1){
                        segments.add(Straight(
                            startPoint = points.last().getSubtracted(points[points.size - 2]).getMultiplied(1.0-startTurnPercent).getAdded(points[points.size -2]),
